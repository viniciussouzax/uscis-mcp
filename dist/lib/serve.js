...
import { envelope, toolError, toolText } from "./envelope.js";
/**
 ...
 */
export async function serve(toolName, args, fetcher) {
    ...
    try {
        ...
        if (stale) {
            return toolText(envelope(stale.value.payload, stale.value.sourceUrl, {
                stale: true,
                staleReason: `upstream_unavailable: ${err.message}`,
                ageSeconds: Math.floor(stale.ageMs / 1000),
            }));
        }
        return toolError(`${toolName} failed: ${err.message}`);
    }
}
