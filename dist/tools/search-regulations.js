...
export const searchRegulationsSchema = {
    ...
    description: "Search Title 8 of the Code of Federal Regulations (USCIS regulations) " +
    ...
    inputSchema: {
        ...
        properties: {
            query: {
                type: "string",
                description: 'Free-text search query. Examples: "H-1B specialty occupation", "adjustment of status eligibility".',
            },
            max_results: {
                type: "integer",
            ...
            cfr_part: {
                type: "string",
                description: 'Optional: restrict to a specific Part of Title 8, e.g. "214" for nonimmigrant classifications.',
            },
        },
        required: ["query"],
    },
...
export async function searchRegulationsHandler(input) {
    ...
    try {
        ...
        if (stale) {
            return toolText(envelope(stale.value, "cached", {
                stale: true,
                staleReason: `upstream_unavailable: ${err.message}`,
                ageSeconds: Math.floor(stale.ageMs / 1000),
            }));
        }
        return toolError(`search_regulations failed: ${err.message}`);
    }
}
//# sourceMappingURL=search-regulations.js.map