...
app.use((req, res, next) => {
    ...
    if (HOST === "127.0.0.1" || HOST === "localhost") {
        ...
        const ok = host.startsWith("localhost") ||
...
app.listen(PORT, HOST, () => {
    ...
    console.log(`[uscis-mcp] HTTP transport listening on http://${HOST}:${PORT}/mcp` +
        (AUTH_TOKEN ? "  (bearer auth enabled)" : "  (no auth)"));
    if (HOST === "0.0.0.0" && !AUTH_TOKEN) {
        console.warn("[uscis-mcp] WARNING: bound to 0.0.0.0 without AUTH_TOKEN — the server is reachable from anywhere on your network without authentication.");
    }
});
