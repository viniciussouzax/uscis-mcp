...
export async function getVolumeIndex() {
    ...
    if (volumes.length === 0) {
        throw new Error(`Found no I&N Dec. volume links on ${LANDING_URL} — page structure may have changed`);
    }
    cache.set(cacheKey, volumes, TTL.SEVEN_DAYS);
    return volumes;
}
export async function getVolumeDecisions(ref, latestVolume) {
    ...
    $("table.no-background").each((_, tableEl) => {
        ...
        rows.each((_, rowEl) => {
            ...
            if (cells.length < 2)
                return; // separator rows (<td colspan=2><hr>)
        ...
        if (decisionRows.length === 1) {
            const summaryParts = [];
            $table.nextUntil("hr, table").each((_, sib) => {
...
export async function loadDecisions(volume) {
...
export async function searchBiaDecisions(query, opts = {}) {
...
export async function getBiaDecisionText(query) {
    ...
    if (query.id) {
        ...
        if (!match) {
            throw new Error(`No decision found with interim decision number "${id}". ` +
        ...
        if (!match) {
            throw new Error(`No decision matching "${query.citation}" found in volume ${volume}. ` +
    ...
    const cached = cache.get(cacheKey);
    if (cached)
        return { payload: cached, sourceUrl: match.pdf_url };
    ...
    if (!contentType.includes("pdf") && !body.subarray(0, 5).toString().startsWith("%PDF")) {
        throw new Error(`Expected a PDF at ${match.pdf_url} but got content-type "${contentType}"`);
    }
