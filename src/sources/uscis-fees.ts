...
export interface FormFeeEntry {
  /** The circumstance this amount applies to, as printed in the schedule. */
  condition: string;
  /** One or more amounts, e.g. ["Paper Filing: $1,440", "Online Filing: $1,390"]. */
  amounts: string[];
}
...
export interface FormFees {
  form_id: string;
  form_title: string;
  /** Edition of the G-1055 schedule these amounts came from, e.g. "05/29/26". */
  edition_date?: string;
  entries: FormFeeEntry[];
  /** The form's block of the schedule, verbatim, for verification. */
  text: string;
}
...
export async function getFormFees(
  formId: string,
): Promise<{ payload: FormFees | null; sourceUrl: string }> {
 ...
 */
async function getFeeTable(): Promise<Map<string, FormFees>> {
  const cacheKey = "uscis:fee-schedule";
  const cached = cache.get(cacheKey) as Array<[string, FormFees]> | null;
...
export function parseFeeSchedule(pdfText: string): Map<string, FormFees> {
  const edition = pdfText.match(/Form G-1055 Edition\s+([\d/]+)/i)?.[1];
  const lines = pdfText.split("\n").map((l) => l.trim());

  const starts: number[] = [];
  lines.forEach((l, i) => {
    if (FORM_LINE.test(l)) starts.push(i);
  });

  const forms = new Map<string, FormFees>();

  for (let s = 0; s < starts.length; s++) {
    const id = lines[starts[s]];
    if (forms.has(id)) continue; // first block wins
...
function parseEntries(rows: string[]): FormFeeEntry[] {
  const entries: FormFeeEntry[] = [];
...
function stripFurniture(line: string): string {
  return (
    line
