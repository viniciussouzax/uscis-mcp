...
function decodeEntities(text: string): string {
  return text.replace(
    /&(?:#x([0-9a-fA-F]{1,6})|#(\d{1,7})|([a-zA-Z][a-zA-Z0-9]{1,9}));/g,
    (match, hex: string | undefined, dec: string | undefined, name: string | undefined) => {
      if (hex !== undefined) return safeCodePoint(parseInt(hex, 16), match);
      if (dec !== undefined) return safeCodePoint(parseInt(dec, 10), match);
