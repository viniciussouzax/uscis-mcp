...
export async function listForms(): Promise<string[]> {
  ...
  const json = await httpGetJson<FormsResp>(url);
...
export async function listFormTypes(formId: string): Promise<FormType[]> {
  ...
  const json = await httpGetJson<FormTypesResp>(url);
...
export async function listOffices(
  ...
  const json = await httpGetJson<OfficesResp>(url);
...
export async function getProcessingTime(args: {
  ...
  const json = await httpGetJson<ProcessingTimeResp>(url);
